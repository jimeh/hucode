...
export function ownsBrowserView(
	info: Pick<IBrowserViewInfo, 'host'>,
	mainWindowId: number,
	environmentService: BrowserViewEnvironment
): boolean {
	if (info.host.windowId !== mainWindowId) {
		return false;
	}
	...
	if (typeof hostedWebContentsId === 'number') {
		return info.host.hostedWebContentsId === hostedWebContentsId;
	}

	return info.host.hostedWebContentsId === undefined;
}
