...
import { NativeManagedSettingsChannelClient } from
	'../../platform/policy/common/nativeManagedSettingsIpc.js';
import {
	IFileManagedSettingsService,
	IManagedSettingsService,
	INativeManagedSettingsService,
} from
	'../../platform/policy/common/copilotManagedSettings.js';
import { FileManagedSettingsChannelClient } from
	'../../platform/policy/common/fileManagedSettingsIpc.js';
...
import { AccountPolicyService, IAccountPolicyGateService } from
...
export class OmniMain extends Disposable {
	...
	private async initServices(): Promise<{
		...
		const policyChannel = this.configuration.policiesData
			? this._register(new PolicyChannelClient(
				this.configuration.policiesData,
				mainProcessService.getChannel('policy')
			))
			: undefined;
		const nativeManagedSettings = this._register(
			new NativeManagedSettingsChannelClient(
				mainProcessService.getChannel('nativeManagedSettings'),
				logService
			)
		);
		serviceCollection.set(
		...
		const fileManagedSettings = this._register(
			new FileManagedSettingsChannelClient(
				mainProcessService.getChannel('fileManagedSettings')
			)
		);
