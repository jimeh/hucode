...
import { RemoteFileSystemProxyServer } from
	'../../platform/files/electron-browser/remoteFileSystemProxyServer.js';
import { RemoteFileSystemProxyClient } from
	'../../platform/files/electron-browser/remoteFileSystemProxyClient.js';
...
export class OmniMain extends Disposable {
	...
	private async initServices(): Promise<{
		...
		// Server: allows other windows to read remote files from this
		// window's providers
		this._register(new RemoteFileSystemProxyServer(
			mainProcessService,
			fileService
		));
		// Client: allows this window to read remote files from other
		// windows' providers
		this._register(RemoteFileSystemProxyClient.register(
			fileService,
			mainProcessService,
			logService,
