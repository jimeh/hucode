...
(async function () {
	...
	type IDesktopMain = import('./omni.main.js').IDesktopMain;
	...
	async function load<M, T extends ISandboxConfiguration>(options: ILoadOptions<T>): Promise<ILoadResult<M, T>> {
		...
		// ESM Import - load the omni-window main module
		try {
			let workbenchUrl: string;
			if (!!safeProcess.env['VSCODE_DEV'] && globalThis._VSCODE_USE_RELATIVE_IMPORTS) {
				workbenchUrl = './omni.desktop.main.js'; // for dev purposes only
			} else {
				workbenchUrl = new URL(`vs/hucode/omni.desktop.main.js`, baseUrl).href;
			}
