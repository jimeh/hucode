...
export class Workbench extends Disposable implements IWorkbenchLayoutService {
	...
	private registerEditorTabHeightClass(): void {
		const updateCompactHeight = () => this.mainContainer.classList.toggle(
			'editor-tabs-compact-height',
			this.editorGroupService.partOptions.tabHeight === 'compact'
		);
		updateCompactHeight();
	...
	/**
	 * Creates the grid descriptor for the Omni shell layout.
	...
	getLayoutClasses(): string[] {
		return coalesce([
			...
			this.isFloatingPanelsEnabled() ? LayoutClasses.MODERN_UI_NOTIFICATIONS_DIALOGS : undefined
		]);
	}
	...
	toggleSecondarySideBar(): void {
		this.setAuxiliaryBarHidden(!this.isSecondarySideBarVisible());
	}
