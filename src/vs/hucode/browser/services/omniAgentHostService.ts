...
import type {
	AuthenticateParams,
	AuthenticateResult,
	AgentHostDebugLogsArtifactKind,
	IAgentCreateSessionConfig,
	IAgentHostDebugLogsArtifact,
	IAgentHostDebugLogsChunk,
...
import type {
	...
	RunAutomationParams,
	RunAutomationResult
} from '../../../platform/agentHost/common/state/protocol/channels-automation/commands.js';
import {
	ComponentToState,
	createRootState,
	RootState,
...
export class OmniAgentHostService implements IAgentHostService {
	...
	dispatch(
		...
		_action: SessionAction | ChatAction | TerminalAction |
			...
			ClientAutomationAction | ClientAutomationRunAction |
			IRootConfigChangedAction
	): void {
		unsupported();
	}
	...
	async listAutomationTriggerDefinitions(
		_params: ListAutomationTriggerDefinitionsParams
	): Promise<ListAutomationTriggerDefinitionsResult> { return unsupported(); }
	async runAutomation(
		_params: RunAutomationParams
	): Promise<RunAutomationResult> { return unsupported(); }
	async fetchAutomationRuns(
		_params: FetchAutomationRunsParams
	): Promise<FetchAutomationRunsResult> { return unsupported(); }
	...
	): Promise<readonly IAgentHostManagedSettingsDiagnostics[]> { return []; }
	async diagnosticsFetch(
		_url: string
	): Promise<IAgentHostNetworkFetchResult> { return unsupported(); }
	async getSessionStateFile(_session: URI): Promise<URI | undefined> { return undefined; }
	async collectDebugLogs(
		_session: URI | undefined,
		_kind: AgentHostDebugLogsArtifactKind
	): Promise<IAgentHostDebugLogsArtifact> { return unsupported(); }
	async readDebugLogsChunk(
		_resource: URI,
		_position: number
	): Promise<IAgentHostDebugLogsChunk> { return unsupported(); }
