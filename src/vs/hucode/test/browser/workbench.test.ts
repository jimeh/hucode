...
suite('Omni Workbench', () => {
	...
	const registerEditorTabHeightClass = Reflect.get(
		Workbench.prototype,
		'registerEditorTabHeightClass'
	) as (this: {
		mainContainer: HTMLElement;
		editorGroupService: {
			readonly partOptions: { readonly tabHeight: string };
			readonly onDidChangeEditorPartOptions: Event<unknown>;
		};
		...
		assert.deepStrictEqual({
			...
			disabled: {
				...
				toggles: [
					...
					{ name: 'modern-ui-notifications-dialogs', force: false },
				],
				styleUpdates: ['titlebar', 'sidebar'],
			},
		});
	...
	test('tracks compact editor tab height on the Omni workbench', () => {
		const optionChanges = disposables.add(new Emitter<unknown>());
		const mainContainer = mainWindow.document.createElement('div');
		let tabHeight = 'compact';
		const host = {
			mainContainer,
			editorGroupService: {
				get partOptions() { return { tabHeight }; },
				onDidChangeEditorPartOptions: optionChanges.event,
			},
		...
		assert.strictEqual(
			mainContainer.classList.contains('editor-tabs-compact-height'),
			true
		);

		tabHeight = 'default';
		optionChanges.fire(undefined);
		assert.strictEqual(
			mainContainer.classList.contains('editor-tabs-compact-height'),
			false
		);
