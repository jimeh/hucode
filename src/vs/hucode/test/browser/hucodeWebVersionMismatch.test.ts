...
suite('HucodeWebVersionMismatch', () => {
	...
	test('shows one blocking alertdialog and reloads the browser tab', async () => {
		let reloads = 0;
		...
		let lateContent: HTMLElement | undefined;
		try {
			...
			lateContent = document.createElement('aside');
			document.body.append(lateContent);
			await Promise.resolve();
			assert.strictEqual(lateContent.inert, true);
			assert.strictEqual(
