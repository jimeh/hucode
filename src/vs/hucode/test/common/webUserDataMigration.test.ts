...
suite('HucodeWebUserDataMigration', () => {
	...
	test('accepts only explicitly allowed bootstrap conflicts', () => {
		assert.strictEqual(isAcceptedWebUserDataResponse(true, 200, false), true);
		assert.strictEqual(isAcceptedWebUserDataResponse(false, 409, true), true);
		assert.strictEqual(isAcceptedWebUserDataResponse(false, 409, false), false);
		assert.strictEqual(isAcceptedWebUserDataResponse(false, 500, true), false);
	});
});
