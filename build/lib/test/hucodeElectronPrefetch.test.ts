...
const checksums = {
	'electron-v42.10.0-darwin-x64.zip': 'darwin-checksum',
	'electron-v42.10.0-linux-armv7l.zip': 'linux-arm-checksum',
};
	...
	test('parses the pinned checksum for the requested Electron artifact', () => {
		...
		assert.strictEqual(
			pinnedDetails.checksums?.[
				'electron-v42.10.0-darwin-x64.zip'
			],
			'248bdf2559e13e570e23bfc1edd639a4ee494a4d592d7fe0843314ead1215c92'
		);
	});
	...
	test('rejects checksums missing the requested Electron artifact', () => {
		assert.throws(
			() => buildElectronArtifactDetails(options, {
				'electron-v42.10.0-linux-x64.zip': 'linux-checksum',
			}),
			/Missing Electron checksum for electron-v42\.10\.0-darwin-x64\.zip/
		);
	});
