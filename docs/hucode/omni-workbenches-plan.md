...
| 2026-07-18 | Exact-head review fixes | Reloaded desktop workbenches reactivated after will-unload, made overlapping closes single-owner, removed false MRU recency from stale web opens, transitioned missing web restores to explicit unloaded state, guarded inactive retained rows, preferred project metadata on restore overlap, and reconciled the selected project alias after promotion | Both review phases are covered by focused regressions; final exact-head re-review and CI remain pending |
...
| 2026-07-18 | Exact-head review regression validation | `typecheck-client`; `compile-client`; focused lifecycle, routing, selection, and tree-model suites | Pass: 126 Electron/browser and 14 tree-model tests; post-will-unload reactivation, overlapping close, stale web recency, missing-restore no-retry, dormant explicit unload, inactive retained rows, project-preferred overlap, and upstream browser-routing branches are covered |
